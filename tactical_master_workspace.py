...
def process_pod_data(pod_name):
    ...
    with ui_container.container():
        ...
        while pool:
            ...
            for t in pool:
                # Prioritize merging into existing routes within 30 miles
                if haversine(anchor['lat'], anchor['lon'], t['lat'], t['lon']) <= 30.0:
                    group.append(t); unique_locs.add(t['full_addr'])
                else:
                    rem.append(t)
            pool = rem
            # NEW LOGIC: Acceptable regardless of stop count if matching states
            clusters.append({"data": group, "center": [anchor['lat'], anchor['lon']], "unique_count": len(unique_locs), "acceptable": True})
...
def render_dispatch_logic(i, cluster, pod_name):
    ...
    with c_btn2:
        if real_gas_id:
            sub = requests.utils.quote(f"Action Required: Route Request | {work_order_title}")
            body = requests.utils.quote(sig)
            mailto = f"https://mail.google.com/mail/?view=cm&fs=1&to={sel_ic['Email']}&su={sub}&body={body}"
...
def run_pod_tab(pod_name):
    ...
    c1.markdown(f"<div class='metric-box'><div class='metric-title'>Total Routes</div><div class='metric-value'>{len(clusters)}</div></div>", unsafe_allow_html=True)
    c2.markdown(f"<div class='metric-box'><div class='metric-title' style='color:{TB_GREEN};'>Ready</div><div class='metric-value'>{len(clusters)}</div></div>", unsafe_allow_html=True)
    ...
    for c in clusters:
        folium.CircleMarker(c['center'], radius=12, color=TB_GREEN, fill=True, opacity=0.8).add_to(m)
    st_folium(m, use_container_width=True, height=450, key=f"map_{pod_name}") # Height shrunken to 450

    for i, cluster in enumerate(clusters):
        with st.expander(f"📍 {cluster['data'][0]['city']}, {cluster['data'][0]['state']} | {cluster['unique_count']} Stops"):
