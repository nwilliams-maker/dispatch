        ...
        while pool:
            ...
            for t in pool:
                if haversine(anchor['lat'], anchor['lon'], t['lat'], t['lon']) <= 30.0:
                    ...
                    rem.append(t)
            pool = rem
            # Initial logic marking: Routes are 'acceptable' if they have enough volume, but we'll re-check IC availability later
...
def render_dispatch_logic(i, cluster, pod_name):
    ...
    if valid_ics.empty:
        ...
        # If we show the UI anyway for manual selection, we proceed, but button should be cautious
        ic_opts = {"MANUAL SEARCH REQUIRED": None}
    else:
    ...
    if ic_opts[sel_label] is not None:
        ...
        st.markdown(f"""
            <div style="background-color: #f8fafc; padding: 12px; border-radius: 8px; border: 1px solid #e2e8f0; margin-bottom: 15px; font-family: 'Roboto', sans-serif !important;">
                ...
                <span style="color: #0f172a; font-weight: 600;">Time: {t_str}</span>
            </div>
        """, unsafe_allow_html=True)

        today_str = datetime.now().strftime("%m%d%Y")
        work_order_title = f"{sel_ic['Name']} - {today_str}-{i}"
        sig = (f"Work Order: {work_order_title}\nContractor: {sel_ic['Name']}\nDue Date: {due.strftime('%A, %b %d, %Y')}\n\nMetrics:\n- Unique Stops: {cluster['unique_count']}\n- Mileage: {mi} mi\n- Time: {t_str}\n- Compensation: ${pay:.2f}\n\nAuthorize here:\n{PORTAL_BASE_URL}?route={link_id}&v2=true")
        st.text_area("Email Payload Preview", sig, height=200, key=f"area_{i}_{pod_name}_{link_id}")

        c_btn1, c_btn2 = st.columns(2)
        with c_btn1:
            if not real_gas_id:
                if st.button("☁️ Sync Data", key=f"sync_btn_{i}_{pod_name}"):
                    returned_id = sync_to_sheet(sel_ic, cluster['data'], mi, t_str, pay, work_order_title, loc_sum, due)
                    if returned_id: st.session_state[sync_key] = returned_id; st.rerun()
            else: st.button("✅ Synced", disabled=True, key=f"done_{i}_{pod_name}")

        with c_btn2:
            if real_gas_id:
                mailto = f"https://mail.google.com/mail/?view=cm&fs=1&to={sel_ic['Email']}&su={requests.utils.quote(f'Action Required: Route Request | {work_order_title}')}&body={requests.utils.quote(sig)}"
                st.markdown(f'<a href="{mailto}" target="_blank" style="text-decoration:none;"><div style="background-color:#76bc21;color:white;padding:10px;text-align:center;border-radius:4px;font-weight:bold;">📧 Send Gmail</div></a>', unsafe_allow_html=True)
            else: st.markdown('<div style="background-color:#e2e8f0;color:#94a3b8;padding:10px;text-align:center;border-radius:4px;font-weight:bold;">📧 Send Email (Sync First)</div>', unsafe_allow_html=True)
    else:
        st.warning("Select a contractor to view financials and email preview.")

def run_pod_tab(pod_name):
    st.markdown(f"<h2 style='font-family: Roboto, sans-serif !important;'>{pod_name} Command Center</h2>", unsafe_allow_html=True)
    if f"clusters_{pod_name}" not in st.session_state:
        if st.button(f"📥 Load {pod_name} Data", key=f"load_{pod_name}"): process_pod_data(pod_name); st.rerun()
        return
    ...
    for c in clusters:
        ...
        # Acceptable if Stop count >= 3 AND we have a nearby IC
    ...
    c1.markdown(f"<div class='metric-box'><div class='metric-title' style='font-family: Roboto !important;'>Total Routes</div><div class='metric-value' style='font-family: Roboto !important;'>{len(clusters)}</div></div>", unsafe_allow_html=True)
    c2.markdown(f"<div class='metric-box'><div class='metric-title' style='color:{TB_GREEN}; font-family: Roboto !important;'>Ready</div><div class='metric-value' style='font-family: Roboto !important;'>{len(acc)}</div></div>", unsafe_allow_html=True)
    c3.markdown(f"<div class='metric-box'><div class='metric-title' style='color:#f44336; font-family: Roboto !important;'>Review</div><div class='metric-value' style='font-family: Roboto !important;'>{len(unacc)}</div></div>", unsafe_allow_html=True)
    if c4.button("🔄 Refresh", key=f"refresh_{pod_name}"): process_pod_data(pod_name); st.rerun()

    m = folium.Map(location=clusters[0]['center'] if clusters else [39.8, -98.5], zoom_start=6, tiles="cartodbpositron")
    for c in acc: folium.CircleMarker(c['center'], radius=12, color=TB_GREEN, fill=True, opacity=0.8).add_to(m)
    for c in unacc: folium.CircleMarker(c['center'], radius=12, color="#f44336", fill=True, opacity=0.8).add_to(m)
    ...
    with t1:
        for i, cluster in enumerate(acc):
            with st.expander(f"📍 {cluster['data'][0]['city']} | {cluster['unique_count']} Stops"):
                render_dispatch_logic(i, cluster, pod_name)
    with t2:
        for i, cluster in enumerate(unacc):
            with st.expander(f"🔴 Review Required | {cluster['data'][0]['city']} | {cluster['unique_count']} Stops"):
...
st.markdown("<h1 style='font-family: Roboto, sans-serif !important;'>Network Command Center</h1>", unsafe_allow_html=True)
